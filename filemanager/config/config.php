...
//include_once '../_app/config/admin.php';
session_start();
...
/*
$_SESSION['RF'] = '';
$_SESSION['RF']['current_path'] = !empty($_REQUEST['current_path']) 
		? $_REQUEST['current_path'] 
			: @$_SESSION['RF']['current_path'];
$_SESSION['RF']['type'] = !empty($_REQUEST['type']) 
		? $_REQUEST['type'] 
			: @$_SESSION['filemanager']['type'];
$_SESSION['RF']['fldr'] = !empty($_REQUEST['fldr']) 
		? $_REQUEST['fldr'] 
			: @$_SESSION['RF']['fldr'];

$_current_path = $_SESSION['RF']['current_path'];
$_type = $_SESSION['RF']['type'];
$_fldr= $_SESSION['RF']['fldr'];	
*/
/*
$upload_dir = !empty($_fldr)
					? $_fldr
						: '/uploads/'; // path from base_url to base of upload folder (with start and final /)
$current_path = !empty($_current_path)
					? $_current_path
						: '../uploads/media_files/'; // relative path from filemanager folder to upload folder (with final /)
*/						
$upload_dir = ''; // path from base_url to base of upload folder (with start and final /)
$current_path = '../uploads/media_files/'; // relative path from filemanager folder to upload folder (with final /)
...
$current_path = '../uploads/media_files/'; // relative path from filemanager folder to upload folder (with final /)
//thumbs folder can't put inside upload folder
$thumbs_base_path = '../uploads/media_files/thumb/'; // relative path from filemanager folder to thumbs folder (with final /)
 * 
 */
//
//print_r($upload_dir);
//print_r($current_path);
//print_r($thumbs_base_path);
...
$ext_video = array('mp4', "flv"); //Video 
$ext_music = array('mp3', 'm4a', 'ac3', 'aiff', 'mid','ogg','wav'); //Audio
$ext_misc = array('zip', 'rar','gz','tar','iso','dmg'); //Archives

//$ext = array_merge($ext_img, $ext_file, $ext_misc, $ext_video,$ext_music); //allowed extensions
//$ext = array_merge($ext_misc,$ext_file,!empty($_type) ? $ext_video : array()); //allowed extensions
$ext = array_merge($ext_misc,$ext_file,$ext_video); //allowed extensions
