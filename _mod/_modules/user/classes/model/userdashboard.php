...
class Model_UserDashboard extends Model_Database {
	...
	public function find_top_count($items='') {
		...
		$where_cond = '';
		if ($items['where_cond'] != '') {
			if (is_array($items['where_cond']) && count($items['where_cond']) != 0) {
				$buffers	= array();
				...
				foreach ($items['where_cond'] as $field => $value) {
					$operator	= '=';
		...
		$sql =  'SELECT '.$fields.' FROM `'.$table.'` WHERE '. $where_cond . $group_by . $order_by . $limit;
