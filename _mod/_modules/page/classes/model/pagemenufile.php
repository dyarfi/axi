...
class Model_PageMenuFile extends Model_Database {
    protected $table = 'page_menu_files'; 
	protected $tbl_name;
	...
	public function load_by_group($id,$field=''){
		
		$where_cond	= array('menu_id' => $id);
		if (!empty($field)) {
			$where_cond = array_merge($where_cond,array('field_name'=>$field));
		}
