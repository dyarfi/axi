...
class Controller_Backend_CareerApplicant extends Controller_Backend_BaseAdmin {
	...
	public function download ($file_name = '') {
		$where_cond	= array('cv_file'	=> $file_name);

		$files		= Career_Applicant_Model::instance()->find($where_cond);

		foreach ($files as $row) {
			download::force(Lib::config($this->_class_name.'.upload_url_cv').$row->cv_file);
		}
	}
