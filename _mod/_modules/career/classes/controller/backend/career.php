...
class Controller_Backend_Career extends Controller_Backend_BaseAdmin {
    ...
    public function action_add() {       
		...
		if ($_POST) {
			...
			$post->rule('requirements', 'not_empty');			
			...
			if ($post->check()) {
				...
				$params		= array('division_id'		=> !empty($fields['division_id']) ? $fields['division_id'] : '',
									...
									'sent_to'			=> isset($fields['sent_to']) ? $fields['sent_to'] : '',
									'ref_no'			=> isset($fields['ref_no']) ? $fields['ref_no'] : '',
									'start_date'		=> isset($fields['start_date']) ? $fields['start_date'] : '',
									'end_date'			=> isset($fields['end_date']) ? $fields['end_date'] : '',
									'report_to'			=> isset($fields['report_to']) ? $fields['report_to'] : '',
									'job_purpose'		=> isset($fields['job_purpose']) ? $fields['job_purpose'] : '',
									'responsibilities'	=> isset($fields['responsibilities']) ? $fields['responsibilities'] : '',
									'requirements'		=> isset($fields['requirements']) ? $fields['requirements'] : '',
    ...
    public function action_edit() {
		...
		if ($_POST) {
			...
			if ($post->check()) {
				...
				$this->request->redirect(ADMIN.$this->_class_name.'/view/'.$this->career->id);
				return;
			} else {
				$fields		= Arr::overwrite($fields, $post->as_array());
				$errors 	= Arr::overwrite($errors, $post->errors('validation'));
	...
	public function action_delete() {
		...
		if ($this->request->is_ajax()) {
			...
			if($this->career->delete($this->id1)) {
				echo 1;
			}	
			else {
				echo 0;	
