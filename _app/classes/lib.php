...
class Lib extends Controller {
	...
	* @return	array
	*/	
	public static function translate($objects = array(), $model = '') {
	...
	*/	
	...
	public static function traverse($aclass='',$url='',$parent_id='',$data='') {
		if (!is_array($data))
			return array();
		...
		foreach ($data as $child) {
			$class  = ($child->parent_id != $parent_id) ? '' :'active';
			$aclass = (Request::$current->param('id1') == $child->title) ? 'active' : '';
			$html .= '<li class="'.$class.'" style="margin:0 0 0 '.($child->sub_level * 20).'px"><a href="'.$url.'/'.$child->title.'" title="'.$child->subject.'" class="'.$aclass.'">
						<b>'.HTML::chars($child->subject ,TRUE);
				
				if ($child->parent_id == $parent_id) {
					$returns[]	= $child;
					$returns	= array_merge_recursive($returns, (array) self::traverse('','',$child->id, $data));
				}				
