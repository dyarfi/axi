...
class Acl extends Controller {
	...
	public function __construct () {
		...
		if (Session::instance()->get('user_id') != '') {
			...
			$this->module_request		= $this->request->controller() .'/'. $this->request->action();	
		...
		if (strpos(Session::instance()->get('curr_url'), ADMIN) !== FALSE 
			...
			Session::instance()->set('curr_url', $this->request->detect_uri());
		}
		
		//print_r(strpos($this->request->detect_uri(), ADMIN));
		// Set previous URL from previous url session
		$this->previous_url	= Session::instance()->get('prev_url');		
		if ($this->user === FALSE  
				&& strstr(ADMIN,$this->request->detect_uri()) === FALSE 
				&& $this->request->controller() !== 'authentication') {
	...
	*/
	public function user_history ($controller='',$action='') {
		...
		if ($this->user !== FALSE) {
			...
			$index				= $this->request->param('id1');
						
			$_instances			= 'Model_'.ucfirst($class_name);
			
			$history			= 'Listings';
			
			if (class_exists('Model_'.ucfirst($class_name))) {
				$model				= new $_instances;					
				if (!empty($index)) {
					if (!empty($model->load($index)->subject)) {
						$history			= !empty($model->load($index)->subject) ? $model->load($index)->subject : '';
					} else if (!empty($model->load($index)->title)) {
						$history			= !empty($model->load($index)->title) ? $model->load($index)->title : '';
					} else if (!empty($model->load($index)->name)) {
						$history			= !empty($model->load($index)->name) ? $model->load($index)->name : '';
					}
				}	
			}
			...
			if (empty($history_position)) {
				$params = array(
								...
								'action' => ucfirst($action). ' on ' . $history,
								'time' => $this->now,
								);
	...
	*/
	public function check_module_permission ($controller='',$action='') {
		...
		if ($module_list && $module_function_list && strstr($this->request->detect_uri(), ADMIN) !='') {
			...
			if ($disable_accessible === FALSE 
				...
				$this->request->redirect(ADMIN . $controller);
	...
	*/	
	public function admin_system_modules () {
		...
		// Check admin url
		if (strpos($this->request->detect_uri(),ADMIN) !== FALSE) {	
			// Get module listings
